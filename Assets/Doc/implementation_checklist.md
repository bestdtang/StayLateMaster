...
- [x] `UI/InterestBubbleView.cs`：显示当前 Topic 图标并订阅兴趣变化。
...
- [x] 菜单 Scene UI：仅 **Start** 进入主玩法（`StartMenuController`）；**不含教程**。
...
- [x] `SampleScene` 内联 **TutorialPage** + `TutorialPageView`：进入玩法 Scene 后默认打开；**底部开始** → 隐藏教程 → **321 倒计时** → `BeginRun()`；**顶部退出** 此时隐藏。
- [x] 局内复查教程（`ShowForReview()`）：显示 **顶部退出**、隐藏底部开始；点退出关闭 overlay 并 `ResumeFromTutorial()`。
- [ ] 倒计时的时候需要隐藏连击数字，三个bar都归零，隐藏喜好气泡
- [ ] 主玩法 Scene 常驻 HUD 按钮（**不做暂停层**）：**教程**（`ShowForReview()`）、**退出**（`LoadScene` 回菜单）。
- [ ] 胜负全屏页、火热结束缓冲期间隐藏或禁用局内教程/退出 HUD 按钮。

### C3. 结尾全屏页（胜/负分离）

- [ ] 胜利与失败分别打开**不同的全屏页面**，响应 `RunController.OnGameWon` / `OnGameLost`。
- [ ] **失败页**：仅结算并展示**当下的快乐值**（`HappinessMeter.Value`）；不展示疲劳、连击、火热次数、眨眼率等其他数据。
- [ ] **胜利页**：不做数值结算，仅呈现胜利演出/文案即可。
- [ ] 两页共用操作：**重来**（`RunController.RestartRun()`，留在主玩法 Scene 直接再开一局）、**回到主菜单**（`SceneManager.LoadScene` 回菜单 Scene）。
...
- [ ] 同步 `Assets/Doc/game_design_spec.md` §17：结算页改为胜/负分离全屏页；失败仅展示当下快乐值，胜利无数值结算；两页提供「重来 / 回到主菜单」。
